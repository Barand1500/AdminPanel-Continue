...
import { sekmeAyarlariOku, sekmeTabCssDegiskenleri, splitSekmeleriHesapla } from '@/admin/baslat-menusu/sistem/sekme-yonetimi/yardimci';
import {
  kenarlikAyariOku,
...
function AdminPanelGovde() {
  ...
  const panelStil = useMemo(
    () => ({
      ...sekmeTabCssDegiskenleri(sekmeAyarlari),
      ...kenarlikRenkCssDegiskeni(kenarlikAyar, tema),
    }),
    [sekmeAyarlari, kenarlikAyar, tema]
  );
  ...
  return (
    ...
    <div
      className={`admin-panel flex h-screen min-h-0 w-full flex-col overflow-hidden ${sekmeAyarlari.sekmeYerlesim === 'kare' ? 'admin-panel--sekme-kare' : ''}${kenarlikAyar.neon ? ' admin-panel--kenarlik-neon' : ''}`}
      data-tema={tema}
      style={panelStil}
    >
      <AdminHeader
        ...
        sekmeYerlesim={sekmeAyarlari.sekmeYerlesim}
        sekmeAyarlari={sekmeAyarlari}
      ...
      <AltAksiyonCubugu
        ...
        onModulSec={modulAcHandler}
        modulAramaAktif={sekmeAyarlari.sekmeAramaAktif}
