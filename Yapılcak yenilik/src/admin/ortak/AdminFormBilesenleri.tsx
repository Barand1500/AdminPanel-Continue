...
export function AdminSekmeler<T extends string>({
  ...
  sekmeler: { id: T; etiket: string; ikon?: string; pasif?: boolean; pasifAciklama?: string }[];
  aktif: T;
    ...
    <div className="ap-sekme-cubugu">
      {sekmeler.map((s) => (
        <button
          ...
          type="button"
          disabled={s.pasif}
          title={s.pasif ? s.pasifAciklama : undefined}
          onClick={() => {
            if (!s.pasif) onDegistir(s.id);
          }}
          className={`ap-sekme ${aktif === s.id ? 'ap-sekme-aktif' : ''} ${s.pasif ? 'ap-sekme-pasif' : ''}`}
        >
