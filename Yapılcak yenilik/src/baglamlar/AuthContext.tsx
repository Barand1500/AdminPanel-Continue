...
import { kullaniciPanelAyarlariYukle } from '@/admin/ortak/api/kullaniciAyarlariApi';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (BACKEND_YOK) {
      ...
      const offline = offlineKullanici();
      setKullanici(offline);
      void kullaniciPanelAyarlariYukle(offline.id);
      setYukleniyor(false);
      return;
    }
    ...
    benGetir()
      .then((k) => {
        setKullanici(k);
        return kullaniciPanelAyarlariYukle(k.id);
      })
      .catch(() => tokenSil())
  ...
  async function giris(email: string, sifre: string) {
    ...
    await kullaniciPanelAyarlariYukle(sonuc.kullanici.id);
  }
