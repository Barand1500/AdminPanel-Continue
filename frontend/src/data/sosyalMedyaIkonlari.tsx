...
/** Simple Icons tabanli 24x24 path verileri */
...
function InstagramArkaPlan({ varyant }: { varyant: 'brand' | 'solid' }) {
  if (varyant === 'solid') {
    return (
      <>
        <defs>
          <linearGradient id="sosyal-ig-solid" x1="0%" y1="100%" x2="100%" y2="0%">
        ...
        <circle cx="12" cy="12" r="12" fill="url(#sosyal-ig-solid)" />
      </>
    );
  }
  ...
  return (
    <>
      <defs>
        <linearGradient id="sosyal-ig-brand" x1="0%" y1="100%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#f09433" />
          <stop offset="25%" stopColor="#e6683c" />
          <stop offset="50%" stopColor="#dc2743" />
          <stop offset="75%" stopColor="#cc2366" />
      ...
      <rect width="24" height="24" rx="6" fill="url(#sosyal-ig-brand)" />
    </>
  );
}
...
export function SosyalIkonSvg({
  ...
  const snapchatBeyaz = platform === 'snapchat';

  if (varyant === 'solid') {
    return (
      <svg className={className} viewBox="0 0 24 24" aria-hidden>
        {platform === 'instagram' ? (
          <InstagramArkaPlan varyant="solid" />
        ) : (
          <circle cx="12" cy="12" r="12" fill={platformRenk} />
        )}
        <IkonGlyph
          platform={platform}
          fill={snapchatBeyaz ? '#000000' : '#ffffff'}
          inset
        />
      </svg>
    );
  ...
  if (varyant === 'brand') {
    return (
      <svg className={className} viewBox="0 0 24 24" aria-hidden>
        {platform === 'instagram' ? (
          <InstagramArkaPlan varyant="brand" />
        ) : (
