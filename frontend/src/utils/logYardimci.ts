...
import type { AdminLogKayit } from '@/features/admin/adminSistemApi';
...
export function loglariGrupla(loglar: AdminLogKayit[]): { grup: string; kayitlar: AdminLogKayit[] }[] {
  ...
  for (const log of loglar) {
    const grup = logTarihGrubu(log.olusturma);
...
export function logKayitOzet(log: AdminLogKayit): LogMesajOzet {
  const parcali = logMesajiAyikla(log.islem);
  return {
    ...parcali,
    modulId: parcali.modulId ?? log.modulId ?? undefined,
    aksiyonId: parcali.aksiyonId ?? log.aksiyonId ?? undefined,
  };
}
