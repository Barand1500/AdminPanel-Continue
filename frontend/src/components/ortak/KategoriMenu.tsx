...
function AltKategoriListesi({
  ...
  return (
    <ul className={derinlik > 0 ? 'mt-1 space-y-0.5 border-l border-slate-100 pl-3' : 'space-y-1'}>
      {liste.map((kat) => (
        <li key={kat.id}>
          {kat.yol ? (
            <Link
              to={kat.yol}
              onClick={onSec}
              className="block rounded-md px-2 py-1.5 text-sm text-slate-600 transition hover:bg-accent hover:text-primary"
            >
              {kat.baslik}
            </Link>
          ) : (
            <span className="block px-2 py-1 text-xs font-bold uppercase tracking-wide text-slate-400">
              {kat.baslik}
            </span>
          )}
        ...
        {acik && (
          <>
            <div
              ...
              aria-hidden
            />
            <aside className="fixed left-0 top-0 z-50 flex h-full w-[min(320px,90vw)] flex-col bg-white shadow-2xl">
  ...
  if (acilisModu === 'liste') {
    return (
      <div ref={ref} className="relative hidden shrink-0 sm:block">
        {tetikleyici}
        {acik && (
          <div className="absolute left-0 top-full z-50 mt-2 w-[min(300px,calc(100vw-2rem))] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                ...
                <>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {secili.altKategoriler?.map((alt) => (
                      <div key={alt.id} className="rounded-xl border border-slate-100 bg-slate-50/50 p-3">
                        {alt.yol ? (
                          <Link
                            to={alt.yol}
                            onClick={kapat}
