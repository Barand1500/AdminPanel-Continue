...
import { HeaderTipSecici } from '@/components/admin/header/HeaderTipSecici';
...
export function HeaderYonetimiFormu() {
  ...
  const sekmeler = useMemo(() => {
    const liste: { id: SekmeId; ad: string }[] = [];
  ...
  const tipDegistir = (tip: HeaderTipi) => {
    ...
    const yeniTanim = headerTipTanimiBul(tip);
    if (!yeniTanim.ustBant && sekme === 'ust-bant') setSekme('logo-gorunum');
    if (!yeniTanim.kategoriArama && sekme === 'kategori-arama') setSekme('logo-gorunum');
    if (!yeniTanim.ekAyarlari && sekme === 'ek-ayarlar') setSekme('logo-gorunum');
  };
  ...
  const gecerliSekme = sekmeler.some((s) => s.id === sekme) ? sekme : sekmeler[0]?.id ?? 'logo-gorunum';
    ...
    <div className="space-y-6">
      <div className="space-y-5">
        ...
        <HeaderTipSecici
          ...
          siteAd={siteAd}
          ayarlar={ayarlar}
          headerAyarlari={{ ...headerAyarlari, headerTipi: aktifTip, tipEk }}
