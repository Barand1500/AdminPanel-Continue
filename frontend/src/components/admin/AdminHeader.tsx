...
interface AdminHeaderProps {
  sekmeler: AdminSekme[]; aktifSekmeId: string; onSekmeSec: (id: string) => void; onSekmeKapat: (id: string) => void; onSekmeleriKapat: (idleri: string[]) => void;
...
export function AdminHeader({ sekmeler, aktifSekmeId, onSekmeSec, onSekmeKapat, onSekmeleriKapat, onSekmeTasi, onSekmeBirlestir, onModulSec, onSekmeAyir, baslatMenuAcik: disBaslatMenuAcik, onBaslatMenuAcikDegistir }: AdminHeaderProps) {
  ...
  return <>
    <header className={`ap-header flex h-12 shrink-0 items-stretch border-b${menuAcik ? ' ap-header--baslat-acik' : ''}`}>
      ...
      <UstSekmeCubugu sekmeler={sekmeler} aktifSekmeId={aktifSekmeId} onSekmeSec={onSekmeSec} onSekmeKapat={onSekmeKapat} onSekmeleriKapat={onSekmeleriKapat} onSekmeTasi={onSekmeTasi} onSekmeBirlestir={onSekmeBirlestir} onSekmeAyir={onSekmeAyir} baslatMenuAcik={menuAcik} />
